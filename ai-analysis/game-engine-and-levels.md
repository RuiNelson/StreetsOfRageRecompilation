...
**Terminology:** the game calls the persistent campaign index `level` and the
within-round index `wave`. In this manuscript, *round* means one of the eight
playable stages, *wave* means the value of `$FFFF04`, and *spawn group* means a
...
1. `game_state` selects the global screen/mode.
2. During gameplay, the low byte of `status_bitfield_1` selects a 12-state level
   pipeline.
3. The camera objects at `$FFE000` and `$FFE100` have their own dispatch states.
4. `wave` indexes both camera boundaries and per-wave resource descriptors.
5. The decompressed enemy-load-cue stream at `$FF6800` drives initial, timed, and
...
- `wave` changes in the camera code, not in `spawn_object_batch`.
...
`game_infinite_loop` at `$000003A2` treats `game_state` (`$FFFF00`) as an even
byte offset into the longword table at `$000003BA`:
...
| `game_state` | Initializer / updater | Role |
|---:|---|---|
...
increment `level`; it exits to round clear, whose state machine performs the
campaign increment.

### 2.2 Frame update order

On the normal, unpaused path, `game_mode_ingame` at `$1087A` performs the
...
The final call is `sub_state_dispatcher` (`$436`). This ordering matters. An
...
The low byte at `$FFFB15` is an even offset into the relative-word table at
`$44C`. The high byte, `$FFFB14`, is reused by art-loading code. Reading the ROM
table gives the exact pipeline:

| Low-byte state | Target | Observed role |
|---:|---:|---|
| `$00` | `$05F4` | select next wave descriptor; prepare art/palettes and initial objects |
| `$02` | `$06A6` | wait until camera corridor is locked, then normalize entities |
| `$04` | `$0784` | consume timed spawn records |
| `$06` | `$0810` | preprocess/filter the next length-prefixed spawn section |
| `$08` | `$0936` | scan deferred records for a spawn whose palette/art can be resident |
| `$0A` | `$0B76` | load required art, then materialize the deferred entity |
| `$0C` | `$0C28` | wait for tracked gameplay entities to drain |
| `$0E` | `$059C` | transitional level-resource reload/fade path |
| `$10` | `$0748` | short wait for a scripted acknowledgement flag |
| `$12` | `$0D70` | staged art-transfer sequencer |
| `$14` | `$04CA` | reload/reset after a player or phase transition |
| `$16` | `$0464` | boss/end-phase music and flow gates |
...
The dispatch is inhibited while `$FFFA1A` is nonzero. That byte/word is also
...
`init_levelstart` at `$106EA` is the destructive boundary between rounds. It:
...
- marks `level_intro_active`;
- initializes the palette fade to `$40`;
- loads the level background, camera, HUD, palettes, and auxiliary effects;
- calls `start_round_setup` to unpack the enemy-load-cue stream and seed entities;
- recreates active P1/P2 objects;
- changes `game_state` from `$28` to `$2A`.

The important calls and their proven effects are:

| Address | Role |
|---:|---|
| `$19848` | clear camera RAM; load/decompress per-round plane data and tile maps; initialize both camera-plane objects |
...
| `$00E5C` | unpack ELC data, select difficulty, seed the spawn stream and initial objects |

### 3.2 Round banner state machine

After initialization, `game_mode_levelstart` dispatches through
`level_intro_substate` (`$FFFB48`) and the table at `$11A5C`:
...
`init_ingame` then sets the fade and sub-second clock and advances immediately
...
`level_intro_active` is cleared.

Attract mode is an exception: `init_ingame` itself calls `init_levelstart`,
loads scripted controls, and adjusts `game_state` so that the same gameplay
updater is reused.
...
The code uses several tables indexed by `level`, and they should not be merged
conceptually:

| ROM address | Entry form | Purpose |
|---:|---|---|
| `$1B036` | eight relative words | pointers to Nemesis-compressed enemy-load-cue streams |
| `$1C1C4` | eight relative words | roots of per-round wave/resource-descriptor sections |
| `$1C378` | eight 6-byte records | pointers and small parameters for a separate level resource load |
| `$5F5B8` | per-round relative records | background/plane compressed resources and camera initialization data |
| `$195D8` | words | X boundary reached by successive waves; forward and reverse halves |
| `$11B44` | eight bytes | level BGM IDs |
...
`start_round_setup` indexes the word table at `$1B036`, adds the selected
relative offset to the table base, and calls `nemesisdec_ram` with destination
`$FF6800`. The eight compressed stream starts are:
...
background map data. This is proved by the destination `$FF6800`, the immediate
assignment of `$00FF6800` to `display_list_head`, and the record consumers at
`$784`, `$810`, `$C60`, and `$CC0`.

### 4.3 Per-round wave descriptor roots

At `$1C1C4` are eight offsets relative to `$1C1C4`:
...
is special: `start_round_setup` reads a longword spawn-stream pointer and a word
geometry selector before passing the remaining six bytes to `$F46`. Later wave
loads at `$5F4` use the offset indexed by `wave + 1` and pass it directly to
...
### 4.5 Separate six-byte resource table at `$1C378`

`load_level_data` multiplies `level` by six, reads a longword pointer, calls
...
Players occupy fixed `$80`-byte objects at `$FFB800` and `$FFB880`. General
gameplay entities use 32 `$80`-byte slots beginning at `$FFB900`. The free-slot
test at `$E46` is simply `object+0 == 0`.
...
Both `$C60` and `$CC0` consume the same compact record:
...
`difficulty_copy`; the record is skipped if it requires a higher difficulty.
...
In `spawn_single_object`, byte `+4` is instead treated as a side selector: zero
...
- `display_list_head` to `$FF6800`;
- a geometry index into the four-word table at `$1C1BC`.

The geometry record supplies an X stride and base. `spawn_object_batch` is then
...
After the initial batches, `$784` treats the current stream position as a timed
list:
...
armed. When the timer expires, one free object slot is acquired and `$C60`
...
`$810` handles the next layer of the ELC stream. It reads a word length, saves the
payload pointer, advances `display_list_head` by that length, and filters compact
...
deferred while `$1053E` and `$8454` queue the needed art/palette. Once
`art_array_cue` becomes zero and a slot is free, the entity is spawned and the
...
Enemy-like types in the range `$20..$2A` are classified through `$9350`. When a
spawned type is in the tracked subset, the loader:

- sets object flag bit 3;
- increments the word at `$FFFB1E`.
...
`$19848` creates two related structures:

- primary plane/camera at `$FFE000`;
- secondary plane/camera at `$FFE100`.
...
| `+$02` | current X (16.16; `$FFE002` is its integer word) |
| `+$06` | previous X |
| `+$0A` | X velocity |
| `+$0E` | current Y |
| `+$12` | previous Y |
...
The former CSV names `scene_length` for `$FFE01A` and
`scenario_x_position` for `$FFE01E` were imprecise. The clamp code at `$19074`
proves that they are the primary camera's maximum and minimum X bounds;
`addresses.csv` now records them as `camera_x_max` and `camera_x_min`.
...
`$19570` consumes bit 0 of `$FFFA0D`, increments `wave`, and loads a boundary
from `$195D8`.
...
When `$FFFF` is reached, `wave` is restored and the fade-out flag is set. This
is one of the generic end-of-scroll paths.

### 6.4 Detecting a locked combat arena

`update_camera_scroll_if_needed` at `$6A6` computes:
...
Round index 6 is exceptional. `$18F32` also consumes `$FFFA0D`, increments
`wave`, and drives a vertical transition through flags `$FFFA14..$FFFA16` and
...
Bit 0 of `$FFFA0D` is the common “advance camera/wave phase” event. It is set by:
...
mode. The ordinary camera machine (`$19570`) consumes it for most rounds; round
7 also has the special consumer at `$18F32`.
...
3. reset the round timer through `$195F6`;
4. use the next `$1C1C4` descriptor and remaining ELC stream to prepare the next
...
These are stored as BCD-like display values in `game_timer`, not frame counts;
`milli_second` supplies the `$5A` frame divider.

### 7.3 Boss/end-phase gate

`init_level_state` applies hard-coded thresholds in addition to the data tables:
...
- round 7's late path stops the clock and sets `end_of_level_flag` directly.

`level_flow_handler` at `$464` waits for tracked entities to drain, changes
music/art state, and raises the boss/late-phase flags. `play_level_music` uses
bit 6 of `$FFFA05` to select boss music (`$90` for round 8, `$87` otherwise)
...
`stage_clear_monitor` at `$117FC` bridges late-stage conditions to
`end_of_level_flag`:
...
- round 7 sets `end_of_level_flag` when its special phase is complete;
- alternate scoreboard/versus-related flags route through additional UI logic.

`end_level_player_exit_update` at `$502C`, called from player logic, then removes
ordinary control and walks each active player to the stage's exit target. Once
the player reaches the target, it waits on `$FFFA74`, sets `fade_out_flag`, and
initializes `palette_fade_counter` to `$40`.

After the fade, `ingame_finish_fade` writes `game_state = $18` in normal campaign
play. Round clear later increments `level` and writes `$28` for the next round.
...
|---:|---:|---|
| `$FF6800` | buffer | Nemesis-decoded enemy-load-cue/spawn stream |
| `$FFB800` | `$80` | P1 object |
| `$FFB880` | `$80` | P2 object |
| `$FFB900` | `$1000` | 32 general `$80`-byte entity slots |
| `$FFDCD0` | long/list | pending art array cue; zero means queued art is drained |
| `$FFE000` | structure | primary camera/plane state |
| `$FFE002` | long root | primary camera X 16.16; integer word is named `cam_x` |
| `$FFE01A` | long | primary camera maximum X boundary |
| `$FFE01E` | long | primary camera minimum X boundary |
| `$FFE100` | structure | secondary camera/plane state |
...
| `$FFFA05` | byte | level/spawn flow flags; bits 2–6 gate filtering, drain, boss, and transitions |
| `$FFFA0D` | byte | bit 0: pending wave/camera phase advance |
...
| `$FFFA71` | byte | fade-out request |
| `$FFFA72` | byte | high-level phase/music/art gates |
| `$FFFA73` | byte | round completion / player exit request |
| `$FFFB12` | word | current spawn X base (low byte advanced between columns) |
| `$FFFB14` | word | high byte: art sequencer bits; low byte: level pipeline state |
| `$FFFB16` | word | shared pipeline delay timer |
| `$FFFB1E` | word | tracked scripted gameplay entities alive |
| `$FFFB20` | word | spawn-column X stride |
| `$FFFC14` | long | active ELC stream cursor |
| `$FFFC28` | long | deferred record selected for spawn |
| `$FFFC2C` | long | pointer to current filtered/length-prefixed ELC section |
| `$FFFC30` | long | required resource-list pointer for deferred spawn |
| `$FFFF02` | word | round index, `0..7` |
| `$FFFF04` | word | wave/camera phase index |

One subtlety is address width. Camera values are manipulated as longwords, but
many comparisons access only the integer word. Thus `$FFE002` is both the start
...
|---|---|---:|
| main modes are init/update pairs | `$3A2`, long table `$3BA` | 100% |
| gameplay level pipeline has 12 states | `$436`, ROM words `$44C..$463` | 100% |
| ELC data is Nemesis-decoded to `$FF6800` | `$E5C..$E82` | 100% |
...
| `$FFE01A/$FFE01E` are camera max/min X | clamp at `$19074`, phase update at `$19570` | 100% |
| round 8 reverses camera-boundary direction | `$1957E..$195D0`, second half of `$195D8` | 100% |
| `wave` is a camera/resource phase index | `$5F4`, `$E98`, `$18F32`, `$19570` | 100% |
| `$FFFB1E` counts progression-gating entities | increments `$C60/$CC0`, decrements `$92F0/$9D8C`, zero tests in pipeline | 99% |
| all semantics of each six-byte resource descriptor | overlapping decoder `$F46` | 75% |
| exact semantic name of every `$FFFA05` bit | many distributed producers/consumers | 75% |
...
3. **Round 7 repeated descriptor offsets.** A trace of `wave`, pipeline state,
   camera state, and `$FFFA14..16` during the elevator/vertical sequence would
   prove why offset `$0E` and `$2C` are reused.
4. **Late-stage flags.** `$FFFA05` is clearly a level-flow bitset, but some bits
